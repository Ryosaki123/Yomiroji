...
function SessionSidebar({ sessions, activeId, onSelect, onNew, onRename, onDelete, onOpenCharacters }) {
  const T = window.tr;
    ...
    <aside className="sidebar">
      <div className="sidebar-head">
        <div className="brand">
          ...
          <div className="brand-name">{window.APP_NAME}</div>
        </div>
        <button className="new-sess" onClick={onNew}>
          <span className="ic">＋</span> {T("newPodcast")}
        </button>
        <button className="new-sess" onClick={onOpenCharacters}
          style={{ marginTop: 8, background: "var(--brand-soft, #f3e6df)", color: "var(--brand-ink, #6a3a28)" }}>
          <span className="ic">🎭</span> {T("charactersTitle")}
        </button>
      </div>
        ...
        {sessions.map((s) => {
          ...
          return (
            <div key={s.id} className={"sess-item" + (s.id === activeId ? " active" : "")}
              ...
              <button className="sess-del" title={T("renameSession")}
                onClick={(e) => { e.stopPropagation(); startRename(s); }}>✎</button>
